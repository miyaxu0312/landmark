...
class PRN:
    ...
    def __init__(self, is_dlib=False, prefix='.'):
        ...
        if is_dlib:
            ...
            detector_path = os.path.join(prefix,
                                         'Data/net-data/mmod_human_face_detector.dat')  # pretrained detector parameters
        ...
        self.triangles = np.loadtxt(prefix + '/Data/uv-data/triangles.txt').astype(np.int32)  # ntri x 3   3d face?

        self.uv_coords = self.generate_uv_coords()

    def generate_uv_coords(self):  # 3d uv and get the valid points
    ...
    def dlib_detect(self, image):
        return self.face_detector(image, 1)  # return a rectangle(left,top,right,bottom) and its confidence
    ...
    def process(self, input, image_info=None):
        ...
        '''
        if isinstance(input, str):  # if path imread
            try:
                image = imread(input)
            except IOError:
        ...
        if image_info is not None:  # cropped to generate bounding box
        ...
        cropped_image = warp(image, tform.inverse,
                             output_shape=(self.resolution_inp, self.resolution_inp))  # crop images which use dlib

        # run our net
        # st = time()
        #cropped_pos = self.net_forward(cropped_image)  # genrate position map
        # print 'net time:', time() - st
        """
        # restore the size of image,????
        ...
        """
        return cropped_image, tform, self.resolution_op

    def get_landmarks(self, pos):  # get the kpt in the test images according to the x-y axis of the standard uv file
        '''
        Args:
    ...
    def get_vertices(self, pos):  # get the valid points of 3d face
        '''
        Args:
            pos: the 3D position map. shape = (256, 256, 3).
        Returns:
        ...
        vertices = all_vertices[self.face_ind, :]  # 43867
    ...
    def get_colors(self, image, vertices):
        ...
        '''
        [h, w, _] = image.shape
        vertices[:, 0] = np.minimum(np.maximum(vertices[:, 0], 0), w - 1)  # x    #control the points within the scope
